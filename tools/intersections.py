...
def slice_faces_plane(vertices,
    ...
    if len(cut_faces_quad) + len(cut_faces_tri) == 0:
        ...
        if len(new_faces) == 0:
            ...
            empty = (np.zeros((0, 3), dtype=np.float64), None,
        ...
        final_color = colors[unique]
        final_face = inverse.reshape((-1, 3))

        return final_vert, final_color, final_face
    ...
    if num_tris > 0:
        ...
        new_colors = np.append(new_colors, np.zeros((new_tri_vertices.shape[0], 4)), axis=0)
...
def slice_mesh_plane(mesh,
    ...
    for origin, normal in zip(plane_origin.reshape((-1, 3)),
        ...
        vertices, colors, faces = slice_faces_plane(vertices=vertices,
                                                    colors=colors,
                                                    faces=faces,
                                                    plane_normal=normal,
                                                    plane_origin=origin,
    ...
    return Trimesh(vertices=vertices, faces=faces, vertex_colors=colors, process=True)