...
Base model: OneBlob + Hash encoding; OneBlob + Geometric features
rendering: neuralRGBD
Loss : RGB, depth, sdf, fs, eikonal (Not working very well)

This is the updated version, scene representation is the same as Co-SLAM
'''
...
def query_color(query_points, geo_feat, embed_pos_fn, color_net):
    query_points_flat = torch.reshape(query_points, [-1, query_points.shape[-1]])
    geo_feat_flat = torch.reshape(geo_feat, [-1, geo_feat.shape[-1]])
    embedded_pos = embed_pos_fn(query_points_flat)

    input_feat = torch.cat([embedded_pos, geo_feat_flat], dim=-1)
    rgb_flat = color_net(input_feat)

    rgb = torch.reshape(rgb_flat, list(query_points.shape[:-1]) + [rgb_flat.shape[-1]])
...
class HashSurface(nn.Module):
    ...
    def __init__(self, config, poses, nabla=True, num_frames=None):
        ...
        self.model = ColorSDFNet_v2(config, 
                    ...
                    input_ch_views=self.input_ch_pos,
    ...
    def query_color_sdf(self, query_points, view_dir=None):
        ...
        return self.model(embed, embe_pos, embedded_dirs=embe_pos)
